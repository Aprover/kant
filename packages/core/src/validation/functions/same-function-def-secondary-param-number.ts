...
export const sameFunctionDefSecondaryParamCardinality = {
    sameFunctionDefSecondaryParamCardinality: (
                        ...
                        if (i.keys.length !== functionDefSecondParamCard.get(i.invoked.ref?.name!)) {
                            if (functionDefSecondParamCard.get(i.invoked.ref?.name!) === 0) {
                                accept(
                                    `error`,
                                    `"${i.invoked.ref
                                        ?.name!}" does not requires any secondary arguments to be passed, but you provided "${
                                        i.keys.length
                                    }".`,
                                    { node: i }
                                )
                                ...
                                accept(
                                    `error`,
                                    `"${i.invoked.ref?.name!}" requires "${functionDefSecondParamCard.get(
                                        i.invoked.ref?.name!
                                    )}" secondary arguments to be passed, but you provided "${i.keys.length}".`,
                                    { node: i }
                                )
                            }
