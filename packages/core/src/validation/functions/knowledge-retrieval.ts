...
export const knowledgeRetrieval = {
    knowledgeRetrieval: (knowledgeClass: KnowledgeClass, protocol: Protocol, accept: ValidationAcceptor): void => {
            ...
            .forEach(kd => {
                if (isKnowledgeDefBuiltin(kd)) {
                    ...
                    for (let i = 0; i < kd.name.length; i++) {
                        ...
                        if (
                            kd.$container.$type === "PrincipalKnowledgeDef" ||
                            kd.$container.$type === "SharedKnowledgeDef"
                        ) {
                ...
                if (isKnowledgeDefCustom(kd)) {
                    ...
                    if (isKnowledgeFromFunction(kd.value)) {
                        ...
                        if (functionName === "PW_HASH" || functionName === "HASH" || functionName === "PUB_GEN") {
                        ...
                        if (functionName === "DF") {
                            if (isKnowledgeRef(kd.left)) {
                                ...
                                if (isKnowledgeFromFunctionArgsElements(kd.value.args)) {
                                    for (let i = 0; i < functionParam.length; i++) {
                                        ...
                                        if (isKnowledgeRef(x)) {
                                            paramName = x.ref
                                            if (
                                                knowledgeClass.getGlobalKnowledgeDescriptorMap().get(x.ref) ===
                                                undefined
                                            ) {
                                                accept(
                                                    "error",
                                                    `The param "${x.ref}" is invoked before been defined.`,
                                                    { node: x }
                                                )
                                            }
                                            arrayParam.push(
                                        ...
                                        if (isListAccess(x)) {
                                            ...
                                            if (
                                                knowledgeClass.getGlobalKnowledgeDescriptorMap().get(paramName) ===
                                                undefined
                                            ) {
                                                accept(
                                                    "error",
                                                    `The param "${paramName}" is invoked before been defined.`,
                                                    { node: x }
                                                )
                                            }
                                            arrayParam.push(
                                ...
                                knowledgeClass.addAliasDiffieHellman(
                                    arrayParam,
                                    knowledgeName,
                                    principalList,
                                    returnType
                        ...
                        if (functionName === "CONCAT") {
                            if (isKnowledgeRef(kd.left)) {
                                ...
                                for (let i = 0; i < tempArr.length; i++) {
                                    ...
                                    let index = 0
                                    if (e) {
                                        let desiredType = knowledgeClass
                                            .getGlobalKnowledgeDescriptorMap()
                                            .get(e)
                                            ?.getType()
                                ...
                                if (isKnowledgeFromFunctionArgsElements(kd.value.args)) {
                                    ...
                                    for (let i = 0; i < functionParam.length; i++) {
                                        ...
                                        if (isKnowledgeRef(x)) {
                                            ...
                                            knowledgeClass.addAliasGlobalKnowledge(
                                                knowledgeSplit,
                                                x.ref,
                                                principalList,
                                                returnType
                                ...
                                if (isKnowledgeFromFunctionArgsElements(kd.value.args)) {
                                    ...
                                    for (let j = 0; j < functionSecondaryParam.length; j++) {
                                        ...
                                        if (isKnowledgeRef(keyParam)) {
                                            ...
                                            if (
                                                knowledgeClass.getGlobalKnowledgeDescriptorMap().get(keyParam.ref) ===
                                                undefined
                                            ) {
                                                accept(
                                                    "error",
                                                    `The param "${keyParam.ref}" is invoked before been defined.`,
                                                    { node: keyParam }
                                                )
                                            }
                                        ...
                                        if (isListAccess(keyParam)) {
                                            ...
                                            if (
                                                knowledgeClass.getGlobalKnowledgeDescriptorMap().get(final) ===
                                                undefined
                                            ) {
                                                accept(
                                                    "error",
                                                    `The param "${final}" is invoked before been defined.`,
                                                    { node: keyParam }
                                                )
                                            }
                                ...
                                if (isKnowledgeFromFunctionArgsElements(kd.value.args)) {
                                    ...
                                    if(wrongkey){
                                        if (isKnowledgeRef(x)) {
                                            ...
                                            let firstIndex = knowledgeClass
                                                .getGlobalKnowledgeDescriptorMap()
                                                .get(x.ref)
                                                ?.getFirstIndex()
                                            let desiredType = knowledgeClass
                                                .getGlobalKnowledgeDescriptorMap()
                                                .get(knowledgeClass.getKnowledgebyIndex(firstIndex!, 0)!)
                                                ?.getType()
                                            knowledgeClass.addAliasGlobalKnowledge(
                                                knowledgeName,
                                                x.ref,
                        ...
                        if (functionName === "HKDF") {
                            ...
                            if (isKnowledgeRef(kd.left)) {
                                ...
                                for (let i = 0; i < returnLength; i++) {
                                    knowledgeClass.addNewGlobalKnowledge(
                                        name.concat("[" + i + "]"),
                                        principalList,
                                        "SymmetricKey"
                                    )
