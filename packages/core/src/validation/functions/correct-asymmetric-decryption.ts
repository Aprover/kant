...
export const correctAsymmetricDecryption = {
    correctAsymmetricDecryption: (
        ...
        streamAllContents(protocol)
            ...
            .forEach(i => {
                if(i.invoked.ref?.name==="PKE_DEC"){
                    ...
                    //accept("error",`The result: "${result}"`,{ node: i })
                    if(knowledgeClass.getIndexesParmKeyPairing(result!)!==true){
                        accept("error",`There is not an 'PKE_ENC' function that can be inverted using this cyphertext and key"`,{ node: i })
                    }
