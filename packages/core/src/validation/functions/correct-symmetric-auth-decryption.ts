...
export const correctSymmetricAuthDecryption = {
    correctSymmetricAuthDecryption: (
        ...
        streamAllContents(protocol)
            ...
            .forEach(i => {
                if (i.invoked.ref?.name === "AEAD_DEC") {
                    ...
                    if (
                        knowledgeClass.getIndexesParmKeyPairing(knowledgeClass.getParmKeyPairing(paramskeys)!) !== true
                    ) {
                        accept(
                            "error",
                            `There is not an 'AEAD_ENC' function that can be inverted using this cyphertext and key"`,
                            { node: i }
                        )
                    }
                }
