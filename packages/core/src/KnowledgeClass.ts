...
export class KnowledgeClass {
          ...
          this.printList.push(first) */
                for (let k = 0; k < currentList.size(); k++) {
                    if (currentList.get(k) === root) {
                        ...
                        this._globalKnowledgeDescriptorMap.set(
                            alias,
                            new KnowledgeNodeDescriptor(i, secondIndex - 1, type)
                        )
                        return i
                    }
                }
    ...
    public cloneNodePoiter(alias: string, root: string, namesList: List) {
        ...
        if (this._listNodePointerKnowledge.has(root)) {
            ...
            for (let i = 0; i < tempPointer.length; i++) {
                ...
                this._globalKnowledgeDescriptorMap.set(
                    actualName,
                    new KnowledgeNodeDescriptor(firstIndex!, secondIndex, type!)
                )
            }
        }
    }
    ...
    public setKeyPairing(pubkey: string, privatekey: string) {
        this._keyPairing.set(pubkey, [
            this._globalKnowledgeDescriptorMap.get(pubkey)?.getFirstIndex()!,
            this._globalKnowledgeDescriptorMap.get(privatekey)?.getFirstIndex()!
        ])
        this._keyPairing.set(privatekey, [
            this._globalKnowledgeDescriptorMap.get(privatekey)?.getFirstIndex()!,
            this._globalKnowledgeDescriptorMap.get(pubkey)?.getFirstIndex()!
        ])
    }
